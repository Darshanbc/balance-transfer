...
func (s *SmartContract) CreateWorkspace(stub shim.ChaincodeStubInterface) sc.Response {
	...
	//	fmt.Println("Physical Location: ", string(pyslocAsbytes))

	//	json.Unmarshal(pyslocAsbytes, &m)
	//	fillStruct(m, &physlocation)
	//
	//	newWrkSpace.PhysicalLocation.Location, _ = m["location"].(map[string]interface{})
	// --------------------------------furnitures------------------
	furn := []furniture{}
	furnAsbytes := args[3]
...
func (s *SmartContract) QueryWorkspace(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	//keyAsBytes := locationRange.GetValue()
	...
	//	json.Unmarshal(keyAsBytes, &keyAsString)
...
func (s *SmartContract) CreateConfig(stub shim.ChaincodeStubInterface) sc.Response {
	...
	if orgAsbytes != nil {
		 shim.Error("Configuration already exists")
	}
	atrrValue, _, _ := cid.GetAttributeValue(stub, "Role")
...
//-------------------------------------------------------------
	...
	//------------------------------------------------------------
//	Ports := wrkSpace.NetAsset.IpPorts
//	for _, port := range Ports {
//		port.ConfigurationID = mspID
//	}
//
//	wrkSpace.NetAsset.IpPorts = Ports
//	logger.Info("workspace after updating ports", wrkSpace)
...
func (s *SmartContract) Query(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	resultAsbytes,_:=stub.GetState(args[0])
